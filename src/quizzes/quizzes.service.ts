...
export class QuizzesService {
  ...
  /**
   * Recalculates the score and status of a quiz attempt after an instructor overrides a grade.
   */
  async recalculateAttemptScore(attemptId: string) {
    const attempt = await this.prisma.quizAttempt.findUnique({
      where: { id: attemptId },
    ...
    const percentage = totalPoints > 0 ? Math.round((earnedPoints / totalPoints) * 100) : attempt.score;
    let finalStatus = attempt.status;

    if (!hasPending) {
      finalStatus = percentage >= attempt.quiz.passGrade ? AttemptStatus.PASSED : AttemptStatus.FAILED;
    } else {
      finalStatus = AttemptStatus.PENDING;
    }

    await this.prisma.quizAttempt.update({
      where: { id: attemptId },
      data: { score: percentage, status: finalStatus },
    });

    if (finalStatus === AttemptStatus.PASSED && attempt.status !== AttemptStatus.PASSED) {
      this.eventEmitter.emit('quiz.passed', {
        studentId: attempt.studentId,
        quizId: attempt.quizId,
        score: percentage,
    ...
    if (finalStatus !== AttemptStatus.PENDING && attempt.status === AttemptStatus.PENDING) {
      this.eventEmitter.emit('attempt.evaluated', {
        studentId: attempt.studentId,
        type: 'QUIZ',
        id: attempt.quizId,
