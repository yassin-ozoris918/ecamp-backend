...
async function main() {
  const { validateUniversitySegmentation } = require('./src/common/utils/segmentation-validation.util.js');
  
  // mock state function
  const check = async (state) => {
    try {
      await validateUniversitySegmentation(prisma, { educationLevel: 'UNIVERSITY', ...state });
      return 'VALID';
    } catch (e) {
      return e.message;
    }
